...
static void *_server_thread(void *args)
{
    ...
    while (1) {
        sock_ip_ep_t remote;
        ssize_t res;
        if ((res = sock_ip_recv(&sock, server_buffer, sizeof(server_buffer), SOCK_NO_TIMEOUT,
                                 &remote)) >= 0) {
#if MEASURE_MEAN == 0 
...
static int ip_start_server(void)
{
    ...
    if (thread_create(server_stack, sizeof(server_stack), THREAD_PRIORITY_MAIN - 1,
                      THREAD_CREATE_STACKTEST, _server_thread, NULL, "UDP server") <= KERNEL_PID_UNDEF) {
        server_socket = -1;
...
int main(void)
{
    ...
    DEBUG("MEASURE_MEAN: %i , LOOPBACK_MODE: %i\n",MEASURE_MEAN, LOOPBACK_MODE);
    ...
    for(unsigned int j = MIN_PACKET_SIZE; j < MAX_PACKET_SIZE; j+=STEP_SIZE) {
        ...
        for (unsigned int i = 0; i < NUM_PACKETS; i++) {
            ...
            /* send udp packet */
            sock_ip_send(NULL, &data, j, PROTNUM_IPV6_NONXT, &remote);
